...
我们将\(\operatorname{W}*{x+b=0}\) 视为一个超平面，那么此时就有一个向量位于通过 \( x_0 \) 且方向为 \( \mathbf{W} \) 的直线上，这个向量就是\(\operatorname{x_0}+{tW}\) 

因此我们需要找到t的值，使得\(\operatorname{x_0}+{tW}\) 位于超平面上，也就是\(\operatorname{x_0}+{tW}={0}\)

我们将\(\operatorname{x_0}+{tW}\) 代入\(\operatorname{W}*{x+b=0}\)
即可得到
$$
...
$$
不难看出，\(J(W,b)\)一定是非负的。也就是说，当没有误分类的点时，损失函数的值就是0。错误分类的点越少，损失函数的值就越小。同时，该函数也是一个连续可导函数
## 随机梯度下降法
